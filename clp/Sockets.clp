...
(deffunction socket-send (?host ?port ?msg)
	(bind ?socket (create-socket AF_INET SOCK_STREAM))
	(bind ?connection (connect ?socket ?host ?port))
	...
	(bind ?ans (readline ?connection))
	;(println "[CLIENT] The server said: " ?ans)
	(close-connection ?socket))

;; Rules
	
(defrule Start
	(start-socket-server ?host ?port)
	=>
...
(defrule bind-sock
	...
	=>
	;(println "[SERVER] Bind sock " ?host " " ?port)
	(setsockopt ?fd SOL_SOCKET SO_REUSEADDR 1)
	(modify ?f (bound
		(bind-socket ?fd ?host ?port))))

(defrule listen-sock
	?sss <- (start-socket-server ? ?)
	?f <- (socket
		(fd ?fd)
		(bound ?)
		(listening nil))
	=>
	;(println "[SERVER] Listening " ?fd)
	(retract ?sss)
	(modify ?f
...
"We respond to the client once we get a new line character as signified by ascii 10"
	...
	(bind ?result (eval ?message))
	;(println "[SERVER] eval result " ?result)
	(printout ?name ?result crlf)
